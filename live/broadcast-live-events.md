...
### JW Player Live Event Media Fields
...
Notes
- Manually added fields will not get overwritten by Broadcast Live. 
- It takes a few minutes before u propagate in Applicaster apps due to caching

### Applicaster Zapp Video Types
Zapp will assign assign a video type based on the fields. You can use these to assign 
| Zapp Video Type | Conditions | Comments |  
| ------ | ------ | ------ |
| `live-future` | `VCH.ScheduledStart` is in the future  |  Map to a landing page WITHOUT a player |      
| `live` | `VCH.ScheduledStart`is in the past and `VCH.ScheduledEnd`in the future   | Link to a landingpage WITH a player |   
...
5. Ensure the videos with type `live-future` link to a video landing page *WITHOUT a player*. See here
