...
The video players used in Samsung and LG are not compatible with the way JW Player exposes subtitles: these players expect the language attribute in the manifest. JW Player HLS streams expose only the language name. As a result, the viewer will not be able to choose another subtitle.

The HLS streams need an adjustment on a property level. 
